...
use crate::{components::tissues::nav_list::NavList, pages::Route, stores::account_info::AccountInfo};
...
pub fn nav_bar() -> Html {
    ...
    let name = 
        if let Some(account_info)= use_context::<Rc<AccountInfo>>(){
    ...
    let nav_lists = Route::NAV
        ...
        .map(|routes| {
            ...
            if routes[0] == Route::Account && name.is_some(){
                let name = name.as_ref().expect("some very old things happend to brake here");
                html!{
